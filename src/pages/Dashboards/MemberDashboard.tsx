...
const MemberDashboard: React.FC = () => {
  ...
  const stats = {
    ...
    upcomingSessions: bookings.filter(b => b.status === 'accepted' && new Date(b.mentorSlot?.date || '') >= new Date()).length,
                ...
                {recentBookings.map((booking) => (
                  <div key={booking.id} className="flex items-center gap-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    ...
                    <div className="flex-1">
                      <p className="font-medium text-gray-900 dark:text-white">
                        Session with {booking.mentorSlot?.mentor?.name || 'Unknown Mentor'}
                      </p>
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        Completed on {booking.mentorSlot?.date ? formatDate(booking.mentorSlot.date) : 'Unknown date'}
                      </p>
                    </div>
